...
So i got my Kinect out from a drawer and started a VCV Rack module with the freenect lib.

## Important

🚧 This module is experimental and a work-in-progress. 🚧

For now, the module only supports the use of one Kinect sensor at a time. Also, there should be only one Kinect module placed on the rack. The use of several Kinect modules can lead to unwanted behavior.

Currently, the resulting signal is jittery and glitching, it needs further refining and corrections to be made smoother.

The Kinect device must be plugged into the computer before starting VCV Rack ; and it shouldn't be unplugged until you remove the Kinect module from the rack.

## Requirements

Of course, the module requires a Kinect device to be connected to the PC.

You cannot connect the Kinect to the PC directly, because it uses a specific proprietary connector (USB with an additional power pin for the motor), forbidding to plug it in a normal usb port. You'll either want to take the Kinect apart to rewire it to a generic USB plug (there are tutorials on the web for that), or to simply buy an appropriate adapter (it plugs to the proprietary connector, and to a power socket).
